...
const HeroSection = () => {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 bg-gradient-to-b from-hero-bg via-transparent to-hero-bg z-0 opacity-80"></div>
      <div className="absolute inset-0 bg-gradient-to-r from-hero-bg via-primary/20 to-transparent z-0"></div>
      ...
      <div className="absolute inset-0 z-0 pointer-events-none">
        {particles.map((p) => (
          <motion.div
            key={p.id}
            className="absolute bottom-0 w-1 h-1 rounded-full bg-accent opacity-20 blur-[1px]"
            initial={{
              x: p.x,
              y: "100%",
              opacity: 0,
        ...
        <div className="grid lg:grid-cols-2 items-center gap-10 lg:gap-12">
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start items-center">
              <a
                href="#contact"
                className="w-full sm:w-auto bg-primary hover:bg-primary-dark text-white px-8 py-4 rounded-xl font-bold text-base flex items-center justify-center gap-3 transition-all shadow-xl shadow-primary/20 group active:scale-95"
              >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 md:gap-6 mt-10 pt-8 border-t border-white/10 max-w-2xl mx-auto lg:mx-0">
              {siteData.stats.map((stat, i) => (
                <div key={i} className="flex flex-col items-center lg:items-start">
                  <span className="text-2xl md:text-3xl font-bebas text-accent leading-none">{stat.value}</span>
          ...
          >
            <div className="relative z-10 rounded-[2rem] overflow-hidden border-2 border-white/10 shadow-[0_0_50px_rgba(30,64,175,0.2)] group aspect-square max-h-[440px] mx-auto">
              <img
              ...
              />
              <div className="absolute inset-0 bg-gradient-to-t from-hero-bg/95 via-transparent to-transparent flex items-end p-8">
                <div className="text-white">
                  <h3 className="font-bebas text-3xl mb-1 tracking-wider uppercase">Industrial Safety</h3>
                  <p className="text-accent/70 text-xs font-inter tracking-widest uppercase font-bold">Premium Protection Systems</p>
                </div>
              </div>
            </div>
            {/* Ambient Background Glows */}
            <div className="absolute -top-5 -right-5 w-48 h-48 bg-primary/20 rounded-full blur-[80px] -z-10 animate-pulse"></div>
            <div className="absolute -bottom-5 -left-5 w-64 h-64 bg-accent/10 rounded-full blur-[80px] -z-10 animate-pulse delay-700"></div>
          </motion.div>
        </div>
      </div>
    </section>
