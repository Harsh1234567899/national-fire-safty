...
const Navbar = ({ unscrolledTheme = "light" }) => {
          ...
          <div className="flex justify-between items-center">
            ...
            <Link href="/#home" className="flex items-center gap-3 group relative">
              ...
              <h1 className="flex flex-col font-bebas tracking-wider leading-[1.1]">
                ...
                <span className={`text-base md:text-lg font-bold ${scrolled ? "text-slate-900" : (unscrolledTheme === "dark" ? "text-white" : "text-slate-800")}`}>FIRE SAFETY</span>
              ...
              {siteData.navLinks.map((link) => (
                <Link
                  ...
                  className={`font-bold transition-all uppercase text-xs tracking-[0.2em] relative after:content-[''] after:absolute after:bottom-[-4px] after:left-0 after:w-0 after:h-[2px] after:bg-accent after:transition-all hover:after:w-full ${
                    scrolled ? "text-slate-900 hover:text-primary" : (unscrolledTheme === "dark" ? "text-white hover:text-accent" : "text-slate-700 hover:text-primary")
                  }`}
                >
                  {link.name}
                </Link>
            ...
            <button
              className={`lg:hidden p-2 rounded-full transition-colors ${
                scrolled ? "text-slate-900 hover:bg-slate-100" : (unscrolledTheme === "dark" ? "text-white hover:bg-white/10" : "text-slate-700 hover:bg-slate-100")
              }`}
